...
function handlerRootClick(){
	$('.agree').bind('click',function(){
		child = $('.children'+$(this).val()+' :checkbox').siblings();	
		console.log(child)
		if ($(this).is(':checked')){
				child.prevObject.attr('checked', true)
		}
		else{
				child.prevObject.attr('checked',false);
		}
	})
}
function handlerChildClick(id){
	$('.checkbox-group :checkbox').bind('click',function(){
		child = $(this).parents('.btn-checkbox');
		console.log(child)
		if ($(this).is(':checked')){
			child.find('.agree').attr('checked', true)
		}
		else{
